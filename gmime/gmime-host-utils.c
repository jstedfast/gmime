		   ...
		   size_t buflen, int *herr)
{
	...
	if ((name = inet_ntop (af, addr, buf, buflen)) == NULL) {
		if (errno == ENOSPC)
			return ERANGE;
	...
	memcpy (buf, addr, hostbuf->h_length);
	addr = buf;
	buf += ALIGN (hostbuf->h_length);
	
	/* h_addr_list */
	((char **) buf)[0] = addr;
	((char **) buf)[1] = NULL;
	hostbuf->h_addr_list = (char **) buf;
	...
	if (gethostbyaddr_r (addr, addrlen, af, hostbuf, buf, buflen, herr))
		return 0;
	else
		return errno;
#else
	...
	retval = gethostbyaddr_r (addr, addrlen, af, hostbuf, buf, buflen, &hp, herr);
	if (hp != NULL)
		*herr = 0;
	else
		retval = -1;
	...
	GETHOST_PROCESS (h, hostbuf, buf, buflen, herr);
